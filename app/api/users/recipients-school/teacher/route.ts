...
export async function GET() {
  try {
    ...
    // pobierz nauczycieli, dyrektorów i uczniów z tej samej szkoły
    const recipients = await db.collection("users")
      .find({
        school: currentUser.school,
        _id: { $ne: currentUser._id },
        role: { $in: ["teacher", "director", "student"] }
      })
    ...
    console.error("Błąd w /api/users/recipients-school/teacher:", err);
