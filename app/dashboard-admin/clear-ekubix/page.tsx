  ...
  return (
    <div className="min-h-screen flex flex-col bg-yellow-200 font-semibold text-black">
      ...
      <header className="flex justify-between items-center p-4 bg-yellow-300 font-bold">
        <h1 className="text-xl">Clear eKubix – Reset dziennika</h1>

        <div className="flex items-center gap-4">
          {loading ? (
            <span className="text-sm font-bold">Ładowanie danych...</span>
          ) : error ? (
      ...
      <main className="flex-1 p-8 flex flex-col items-center">
        {error && !loading && <div className="mb-4 text-red-600 font-bold">{error}</div>}

        {confirmStep === 0 && (
          <button
            onClick={handleResetClick}
            className="bg-red-500 text-white p-3 rounded hover:bg-red-600 font-bold"
          >
        ...
        {confirmStep > 0 && (
          <div className="flex flex-col items-center gap-4 max-w-lg text-center font-bold">
            <p className={confirmStep === 3 ? "text-red-800 uppercase" : "text-red-800"}>
              {actionMessage}
            </p>
            <div className="flex gap-4">
              <button
                ...
                className={confirmStep === 3
                  ? "bg-red-800 text-white px-4 py-2 rounded hover:bg-red-900 uppercase font-bold"
                  : "bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 font-bold"}
                disabled={processing}
              >
                TAK
              </button>
              <button
                ...
                className={confirmStep === 3
                  ? "bg-gray-300 text-black px-4 py-2 rounded hover:bg-gray-400 uppercase font-bold"
                  : "bg-gray-300 text-black px-4 py-2 rounded hover:bg-gray-400 font-bold"}
                disabled={processing}
              >
                NIE
              </button>
